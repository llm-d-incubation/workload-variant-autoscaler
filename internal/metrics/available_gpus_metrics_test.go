...
func TestResetAvailableGPUsMetric(t *testing.T) {
	registry := prometheus.NewRegistry()
	if err := InitMetrics(registry); err != nil {
		t.Fatalf("InitMetrics failed: %v", err)
	}
	...
	// Record initial GPU metrics for multiple accelerator types
	emitter.RecordAvailableGPUsMetric("nvidia.com", "NVIDIA-A100-PCIE-80GB", testAcceleratorTypeA100, 8)
	emitter.RecordAvailableGPUsMetric("nvidia.com", "NVIDIA-H100-SXM5-80GB", testAcceleratorTypeH100, 4)
	emitter.RecordAvailableGPUsMetric("nvidia.com", "NVIDIA-V100-PCIE-16GB", "V100", 16)
	...
	var beforeResetCount int
	for _, mf := range metrics {
		if mf.GetName() == constants.WVAAvailableGpus {
			beforeResetCount = len(mf.GetMetric())
		}
	}
	if beforeResetCount != 3 {
		t.Errorf("Expected 3 metric series before reset, got %d", beforeResetCount)
	}

	// Reset all GPU metrics
	emitter.ResetAvailableGPUsMetric()
	...
	var afterResetCount int
	for _, mf := range metrics {
		if mf.GetName() == constants.WVAAvailableGpus {
			afterResetCount = len(mf.GetMetric())
		}
	}
	if afterResetCount != 0 {
		t.Errorf("Expected 0 metric series after reset, got %d", afterResetCount)
	}

	// Record new GPU metrics
	emitter.RecordAvailableGPUsMetric("nvidia.com", "NVIDIA-H100-SXM5-80GB", testAcceleratorTypeH100, 12)

	// Verify only the new metrics are present
	metrics, err = registry.Gather()
	if err != nil {
		t.Fatalf("Failed to gather metrics: %v", err)
	}
	...
	for _, mf := range metrics {
		if mf.GetName() == constants.WVAAvailableGpus {
			found = true
			if len(mf.GetMetric()) != 1 {
				t.Errorf("Expected 1 metric series after reset and new record, got %d", len(mf.GetMetric()))
			}
			m := mf.GetMetric()[0]
			g := m.GetGauge()
			if g == nil {
				t.Error("Expected gauge metric")
			} else if g.GetValue() != 12 {
				t.Errorf("Expected gauge value 12, got %f", g.GetValue())
			}
			acceleratorType := getLabelValue(m, constants.LabelAcceleratorType)
			if acceleratorType != testAcceleratorTypeH100 {
				t.Errorf("Expected accelerator_type=%s, got %s", testAcceleratorTypeH100, acceleratorType)
			}
		}
	}
	if !found {
