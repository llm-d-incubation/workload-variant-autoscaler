...
var _ = Describe("Scale-To-Zero Feature with LeaderWorkerSet", Label("full", "flaky"), Ordered, func() {
	var (
		...
		hpaName          = "scale-to-zero-lws-hpa"
		lwsGroupSize     = int32(2) // 1 leader + 1 worker
	)

	BeforeAll(func() {
		if !cfg.ScaleToZeroEnabled {
			Skip("HPAScaleToZero feature gate is not enabled; skipping scale-to-zero LWS test")
		}
		...
		By("Creating VariantAutoscaling resource with minReplicas=0 for LWS")
		err = fixtures.EnsureVariantAutoscaling(
			ctx, crClient, cfg.LLMDNamespace, vaName,
		...
		GinkgoWriter.Println("Scale-to-zero LWS test setup complete")
	})

	AfterAll(func() {
		By("Cleaning up scale-to-zero LWS test resources")
	...
	Context("Initial state and scale-up", func() {
		...
		It("should scale up LWS under load", func() {
			...
			targetURL := fmt.Sprintf("http://%s-service:8000", modelServiceName)
			err := fixtures.CreateLoadJob(ctx, k8sClient, cfg.LLMDNamespace, "stz-lws-load", targetURL, loadCfg)
			Expect(err).NotTo(HaveOccurred(), "Failed to create load job")

			jobName := "stz-lws-load-load"
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(statusReplicas).To(BeNumerically(">", 1), "LWS should scale up under load")
			}, 5*time.Minute, 10*time.Second).Should(Succeed())

			GinkgoWriter.Println("LWS scaled up under load")

			By("Waiting for load job to complete")
			Eventually(func(g Gomega) {
				job, err := k8sClient.BatchV1().Jobs(cfg.LLMDNamespace).Get(ctx, jobName, metav1.GetOptions{})
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(job.Status.Succeeded+job.Status.Failed).To(BeNumerically(">", 0), "Load job should complete")
			}, 10*time.Minute, 15*time.Second).Should(Succeed())
	...
	Context("Scale-to-zero after idle period", func() {
		It("should scale VA desired replicas to zero when no requests", func() {
			...
			Eventually(func(g Gomega) {
				...
				GinkgoWriter.Printf("VA desired replicas: %d (waiting for 0)\n", desiredReplicas)
				g.Expect(desiredReplicas).To(Equal(int32(0)), "VA should recommend scaling to zero after idle period")
			}, 10*time.Minute, 15*time.Second).Should(Succeed())

			GinkgoWriter.Println("VA recommended scaling to zero")
		})

		It("should scale actual LWS replicas to zero", func() {
			By("Monitoring LWS for scale-down to zero")
			Eventually(func(g Gomega) {
				lws := &unstructured.Unstructured{}
				lws.SetAPIVersion("leaderworkerset.x-k8s.io/v1")
				lws.SetKind("LeaderWorkerSet")
				...
				statusReplicas, _, _ := unstructured.NestedInt64(lws.Object, "status", "replicas")
				GinkgoWriter.Printf("Current LWS replicas: %d (waiting for 0)\n", statusReplicas)
				g.Expect(statusReplicas).To(Equal(int64(0)), "LWS should scale to zero after idle period")
			}, 15*time.Minute, 20*time.Second).Should(Succeed())

			GinkgoWriter.Println("LWS successfully scaled to zero")
		})
...
var _ = Describe("Scale-To-Zero Feature with LeaderWorkerSet - Disabled", Label("full"), Ordered, func() {
	var (
		...
		hpaName           = "scale-to-zero-disabled-lws-hpa"
		lwsGroupSize      = int32(2) // 1 leader + 1 worker
		configMapName     = "wva-model-scale-to-zero-config"
		originalConfigMap *corev1.ConfigMap
	)
...
enable_scale_to_zero: false`, cfg.ModelID),
		...
		err = fixtures.EnsureVariantAutoscaling(
			ctx, crClient, cfg.LLMDNamespace, vaName,
			lwsName, cfg.ModelID, cfg.AcceleratorType, 10.0,
			cfg.ControllerInstance,
			fixtures.WithScaleTargetKind("LeaderWorkerSet"),
		)
		Expect(err).NotTo(HaveOccurred(), "Failed to create VariantAutoscaling")

		By("Creating scaler with minReplicas=1 for LWS (scale-to-zero disabled)")
		if cfg.ScalerBackend == "keda" {
			_ = k8sClient.AutoscalingV2().HorizontalPodAutoscalers(cfg.LLMDNamespace).Delete(ctx, hpaName+"-hpa", metav1.DeleteOptions{})
			err = fixtures.EnsureScaledObject(ctx, crClient, cfg.LLMDNamespace, hpaName, lwsName, vaName, 1, 10, cfg.MonitoringNS,
				fixtures.WithScaledObjectScaleTargetKind("LeaderWorkerSet"))
			Expect(err).NotTo(HaveOccurred(), "Failed to create ScaledObject")
		} else {
			err = fixtures.EnsureHPA(ctx, k8sClient, cfg.LLMDNamespace, hpaName, lwsName, vaName, 1, 10,
				fixtures.WithScaleTargetRefKind("LeaderWorkerSet"))
			Expect(err).NotTo(HaveOccurred(), "Failed to create HPA")
		}

		GinkgoWriter.Println("Scale-to-zero disabled LWS test setup complete")
	})
	...
	It("should maintain minimum replicas when scale-to-zero is disabled", func() {
		...
		By("Verifying LWS maintains at least 1 replica")
		Consistently(func(g Gomega) {
			lws := &unstructured.Unstructured{}
			lws.SetAPIVersion("leaderworkerset.x-k8s.io/v1")
			lws.SetKind("LeaderWorkerSet")
			...
			readyReplicas, _, _ := unstructured.NestedInt64(lws.Object, "status", "readyReplicas")
			g.Expect(readyReplicas).To(BeNumerically(">=", 1),
				"LWS should maintain at least 1 replica when scale-to-zero is disabled")
		}, 2*time.Minute, 10*time.Second).Should(Succeed())

		GinkgoWriter.Println("Verified: LWS maintains minimum replicas with scale-to-zero disabled")
	})
