...
var _ = Describe("Saturation Mode - Single VariantAutoscaling with LeaderWorkerSet", Label("full"), Ordered, func() {
	var (
		...
		hpaName          = "saturation-single-lws-hpa"
	...
	BeforeAll(func() {
		...
		DeferCleanup(func() {
			cleanupResource(ctx, "LeaderWorkerSet", cfg.LLMDNamespace, lwsName,
				func() error {
					...
					lws.SetNamespace(cfg.LLMDNamespace)
					lws.SetName(lwsName)
					return crClient.Delete(ctx, lws)
				},
				func() bool {
					lws := &lwsv1.LeaderWorkerSet{}
					err := crClient.Get(ctx, client.ObjectKey{Namespace: cfg.LLMDNamespace, Name: lwsName}, lws)
					return errors.IsNotFound(err)
				})
		})
		...
		By("Waiting for model service LWS to be ready")
		Eventually(func(g Gomega) {
			lws := &lwsv1.LeaderWorkerSet{}
			err := crClient.Get(ctx, client.ObjectKey{Namespace: cfg.LLMDNamespace, Name: lwsName}, lws)
			g.Expect(err).NotTo(HaveOccurred(), "Should be able to get LWS")

			// Log LWS status for debugging
			if lws.Status.ReadyReplicas < 1 {
				GinkgoWriter.Printf("LWS status: Replicas=%d, ReadyReplicas=%d\n",
					lws.Status.Replicas, lws.Status.ReadyReplicas)

				// Check pod status for more details
				podList, listErr := k8sClient.CoreV1().Pods(cfg.LLMDNamespace).List(ctx, metav1.ListOptions{
					LabelSelector: fmt.Sprintf("leaderworkerset.sigs.k8s.io/name=%s", lwsName),
				})
				if listErr == nil {
					for _, pod := range podList.Items {
						GinkgoWriter.Printf("Pod %s: Phase=%s, Ready=%v\n", pod.Name, pod.Status.Phase, pod.Status.Phase == corev1.PodRunning)
			...
			g.Expect(lws.Status.ReadyReplicas).To(Equal(int32(1)), "Model service LWS should have 1 ready replica")
		}, time.Duration(cfg.PodReadyTimeout)*time.Second, 5*time.Second).Should(Succeed())
		...
		if cfg.ScalerBackend == "keda" {
			// Remove any existing HPA that might manage this LWS (e.g. from a prior run with prometheus-adapter)
			// so KEDA's webhook does not reject the ScaledObject with "workload already managed by HPA"
			_ = k8sClient.AutoscalingV2().HorizontalPodAutoscalers(cfg.LLMDNamespace).Delete(ctx, hpaName+"-hpa", metav1.DeleteOptions{})
	...
	AfterAll(func() {
		...
		// Delete in reverse dependency order: scaler (HPA or ScaledObject) -> VA
		// Service and LWS cleanup is handled by DeferCleanup registered in BeforeAll
		if cfg.ScalerBackend == "keda" {
			Expect(fixtures.DeleteScaledObject(ctx, crClient, cfg.LLMDNamespace, hpaName)).NotTo(HaveOccurred())
		} else {
			hpaNameFull := hpaName + "-hpa"
		...
		It("should have scaler created and tracking VA metrics", func() {
			if cfg.ScalerBackend == "keda" {
				...
				Eventually(func(g Gomega) {
					...
					g.Expect(listErr).NotTo(HaveOccurred())
					for i := range hpaList.Items {
						if hpaList.Items[i].Spec.ScaleTargetRef.Name == lwsName {
							return
						}
					}
					g.Expect(false).To(BeTrue(), "KEDA should have created an HPA for the LWS")
				}, 2*time.Minute, 5*time.Second).Should(Succeed())
			} else {
				By("Verifying HPA exists")
				hpa, err := k8sClient.AutoscalingV2().HorizontalPodAutoscalers(cfg.LLMDNamespace).Get(ctx, hpaName+"-hpa", metav1.GetOptions{})
	...
	Context("Replica stability under constant load", Label("full"), func() {
		It("should maintain stable replica count under constant load", func() {
			...
			targetURL := fmt.Sprintf("http://%s-service:8000", modelServiceName)
			err := fixtures.CreateLoadJob(ctx, k8sClient, cfg.LLMDNamespace, "stability-lws-load", targetURL, loadCfg)
			Expect(err).NotTo(HaveOccurred(), "Failed to create stability load job")

			jobName := "stability-lws-load-load"
			...
			for i := 0; i < 6; i++ {
				...
				err := crClient.Get(ctx, client.ObjectKey{Namespace: cfg.LLMDNamespace, Name: lwsName}, lws)
				Expect(err).NotTo(HaveOccurred())
				replicaCounts = append(replicaCounts, lws.Status.Replicas)
				time.Sleep(30 * time.Second)
			}
			...
			GinkgoWriter.Printf("LWS replica count remained stable at %d under constant load\n", baseline)
		})
	})
})
...
var _ = Describe("Saturation Mode - Multiple VariantAutoscalings with LeaderWorkerSet", Label("full"), Ordered, func() {
	var (
		...
		lwsNameB      = modelServiceB + "-decode"
		vaA           = "saturation-multi-lws-va-a"
		vaB           = "saturation-multi-lws-va-b"
		hpaA          = "saturation-multi-lws-hpa-a"
		hpaB          = "saturation-multi-lws-hpa-b"
		lwsGroupSize  = int32(2) // 1 leader + 1 worker
	)
	...
	BeforeAll(func() {
		...
		// Pool A (cheaper)
		err := fixtures.EnsureModelServiceLWS(ctx, crClient, k8sClient, cfg.LLMDNamespace, modelServiceA, poolA, cfg.ModelID, cfg.UseSimulator, cfg.MaxNumSeqs, lwsGroupSize)
		Expect(err).NotTo(HaveOccurred())

		err = fixtures.EnsureService(ctx, k8sClient, cfg.LLMDNamespace, modelServiceA, lwsNameA, 8000)
		Expect(err).NotTo(HaveOccurred())

		By("Creating ServiceMonitor for service A")
		err = fixtures.EnsureServiceMonitor(ctx, crClient, cfg.MonitoringNS, cfg.LLMDNamespace, modelServiceA, lwsNameA)
		Expect(err).NotTo(HaveOccurred(), "Failed to create ServiceMonitor A")

		// Pool B (more expensive)
		err = fixtures.EnsureModelServiceLWS(ctx, crClient, k8sClient, cfg.LLMDNamespace, modelServiceB, poolB, cfg.ModelID, cfg.UseSimulator, cfg.MaxNumSeqs, lwsGroupSize)
		Expect(err).NotTo(HaveOccurred())

		err = fixtures.EnsureService(ctx, k8sClient, cfg.LLMDNamespace, modelServiceB, lwsNameB, 8000)
		Expect(err).NotTo(HaveOccurred())

		By("Creating ServiceMonitor for service B")
		err = fixtures.EnsureServiceMonitor(ctx, crClient, cfg.MonitoringNS, cfg.LLMDNamespace, modelServiceB, lwsNameB)
		Expect(err).NotTo(HaveOccurred(), "Failed to create ServiceMonitor B")

		By("Waiting for both model services to be ready")
		Eventually(func(g Gomega) {
			lwsA := &lwsv1.LeaderWorkerSet{}
			err := crClient.Get(ctx, client.ObjectKey{Namespace: cfg.LLMDNamespace, Name: lwsNameA}, lwsA)
			g.Expect(err).NotTo(HaveOccurred())
			g.Expect(lwsA.Status.ReadyReplicas).To(Equal(int32(1)))

			lwsB := &lwsv1.LeaderWorkerSet{}
			err = crClient.Get(ctx, client.ObjectKey{Namespace: cfg.LLMDNamespace, Name: lwsNameB}, lwsB)
			g.Expect(err).NotTo(HaveOccurred())
			g.Expect(lwsB.Status.ReadyReplicas).To(Equal(int32(1)))
		}, time.Duration(cfg.PodReadyTimeout)*time.Second, 5*time.Second).Should(Succeed())

		By("Creating two VAs with different costs")
		// VA A: Lower cost (should be preferred)
		err = fixtures.EnsureVariantAutoscaling(ctx, crClient, cfg.LLMDNamespace, vaA, lwsNameA, cfg.ModelID, "A100", 30.0, cfg.ControllerInstance,
			fixtures.WithScaleTargetKind("LeaderWorkerSet"))
		Expect(err).NotTo(HaveOccurred())

		// VA B: Higher cost
		err = fixtures.EnsureVariantAutoscaling(ctx, crClient, cfg.LLMDNamespace, vaB, lwsNameB, cfg.ModelID, "H100", 50.0, cfg.ControllerInstance,
			fixtures.WithScaleTargetKind("LeaderWorkerSet"))
		Expect(err).NotTo(HaveOccurred())

		By("Creating scalers for both LWS (HPA or ScaledObject per backend)")
		if cfg.ScalerBackend == "keda" {
			// Remove any existing HPAs that might manage these LWS so KEDA's webhook accepts the ScaledObjects
			_ = k8sClient.AutoscalingV2().HorizontalPodAutoscalers(cfg.LLMDNamespace).Delete(ctx, hpaA+"-hpa", metav1.DeleteOptions{})
			_ = k8sClient.AutoscalingV2().HorizontalPodAutoscalers(cfg.LLMDNamespace).Delete(ctx, hpaB+"-hpa", metav1.DeleteOptions{})
			err = fixtures.EnsureScaledObject(ctx, crClient, cfg.LLMDNamespace, hpaA, lwsNameA, vaA, 1, 10, cfg.MonitoringNS,
				fixtures.WithScaledObjectScaleTargetKind("LeaderWorkerSet"))
			Expect(err).NotTo(HaveOccurred())
			err = fixtures.EnsureScaledObject(ctx, crClient, cfg.LLMDNamespace, hpaB, lwsNameB, vaB, 1, 10, cfg.MonitoringNS,
				fixtures.WithScaledObjectScaleTargetKind("LeaderWorkerSet"))
			Expect(err).NotTo(HaveOccurred())
		} else {
			err = fixtures.EnsureHPA(ctx, k8sClient, cfg.LLMDNamespace, hpaA, lwsNameA, vaA, 1, 10,
				fixtures.WithScaleTargetRefKind("LeaderWorkerSet"))
			Expect(err).NotTo(HaveOccurred())
			err = fixtures.EnsureHPA(ctx, k8sClient, cfg.LLMDNamespace, hpaB, lwsNameB, vaB, 1, 10,
				fixtures.WithScaleTargetRefKind("LeaderWorkerSet"))
			Expect(err).NotTo(HaveOccurred())
		}
	})
	...
	AfterAll(func() {
		By("Cleaning up multi-variant LWS test resources")
		...
		// Delete LWS
		lwsA := &lwsv1.LeaderWorkerSet{}
		lwsA.SetNamespace(cfg.LLMDNamespace)
		lwsA.SetName(lwsNameA)
		_ = crClient.Delete(ctx, lwsA)
	...
	It("should prefer cheaper variant (VA A) for scale-up when both variants are available", func() {
		...
		jobNameB := "multi-lws-load-b-load"
		...
		GinkgoWriter.Printf("VA A (cheaper, cost=30.0) LWS replicas: %d\n", replicasA)
		GinkgoWriter.Printf("VA B (expensive, cost=50.0) LWS replicas: %d\n", replicasB)
